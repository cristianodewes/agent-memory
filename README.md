...
| **Server** | Spring Boot 4.x (Java 21+, Maven) | Source of truth. Ingest, sanitize, store, consolidate (LLM), recall, handoffs, MCP `/mcp`, `/api/v1`, `/web`, auth. |
...
├── server/                 # Spring Boot app — source of truth (Maven)
...
- **Go 1.23+** (go1.26 confirmed on this machine).
- **JDK 21+** (JDK 25 works) and **Maven 3.9+** (the Maven Wrapper `mvnw` is committed).
